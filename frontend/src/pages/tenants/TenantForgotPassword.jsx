...
export const TenantForgotPassword = () => {
    ...
    const handleForgotPassword = async (e) => {
        ...
        try {
            const response = await axios.post("http://localhost:5000/tenant/send-otp", { email });

            if (response.data.error) {
                setError(response.data.message);
            } else {
                navigate('/tenant/otp', { state: { email } });
            }
        } catch (err) {
    ...
    return (
        <div className="relative bg-gradient-to-br from-[#F6F4EB] via-[#E3DAC9] to-[#D8B258] min-h-screen flex items-center justify-center px-4 sm:px-6 lg:px-8 overflow-hidden">
            ...
            <div className="relative bg-white p-6 sm:p-8 md:p-10 lg:p-12 rounded-3xl shadow-2xl w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg z-10 text-center border border-gray-200 backdrop-blur-lg bg-opacity-90">
                <h2 className="text-2xl sm:text-3xl font-bold text-[#103538]">Forgot Password</h2>
                ...
                <form className="space-y-4" onSubmit={handleForgotPassword}>
                    ...
                    <button
                        type="submit"
                        className="w-full mt-2 bg-[#103538] text-white py-2 sm:py-3 rounded-lg font-semibold hover:bg-[#759B87] transition shadow-lg transform hover:scale-105 hover:shadow-xl text-sm sm:text-base"
                    >
                        Generate OTP
                    </button>
                </form>
